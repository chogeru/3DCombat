...
public class OperationGuideManager : MonoBehaviour
{
    ...
    [Header("UI References"), Header("フェードイン・アウト用のCanvasGroup（シーン上のオブジェクト）"), SerializeField]
    private CanvasGroup m_CanvasGroup;
    [Header("ガイドテキスト（シーン上のオブジェクト）"), SerializeField]
    ...
    [Header("次のガイドまでの間隔"), SerializeField]
    private float m_IntervalTime = 0.5f;

    [Header("Guide Messages"), TextArea, SerializeField]
    private List<string> m_GuideMessages = new List<string>()
    {
        "WSADで移動する",
        "Spaceを押してジャンプする",
    ...
    public void PlayFirstLaunchGuide()
    {
        ...
        if (!PlayerPrefs.HasKey(KEY_HAS_SHOWN_GUIDE))
        {
            ...
            //ガイドを表示する処理を開始（非同期なのでForgetで投げっぱなしにする）
            ShowGuideSequenceAsync(m_GuideMessages).Forget();

            //「もう表示したよ」という証（鍵）を保存する

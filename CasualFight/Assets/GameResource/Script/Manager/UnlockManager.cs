...
{
    [Header("初回解放時に表示するUIパネル等のオブジェクト"), SerializeField]
    private GameObject m_FirstUnlockUI;

    [Header("この解放グループを識別するためのセーブキー"), Tooltip("全対象オブジェクトで共通のメモの名前（キー）にするのが最大のポイントです"), SerializeField]
    private string m_UnlockGroupKey = "HasUnlocked_DefaultGroup";

    [Header("デバッグ用: ゲーム起動時にこのキーのセーブデータをリセットするか"), SerializeField]

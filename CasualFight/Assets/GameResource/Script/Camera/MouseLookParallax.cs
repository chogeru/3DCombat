...
{
    [Header("カメラの向きが変わる最大角度（度）"), Header("左右の最大回転角度"), SerializeField]
    float m_RotationRangeX = 5f;
    [Header("上下の最大回転角度"), SerializeField]
    float m_RotationRangeY = 3f;

    [Header("UI用の動く範囲（ピクセル単位）"), SerializeField]
    float m_UIRangeX = 50f;
    [SerializeField]
    float m_UIRangeY = 30f;

    [Header("動きの滑らかさ"), SerializeField]
    float m_SmoothTime = 5f;

    [Header("パララックス対象リスト"), Tooltip("複数のオブジェクトをそれぞれ異なる強度で動かせます"), SerializeField]
    ParallaxTarget[] m_ParallaxTargets;

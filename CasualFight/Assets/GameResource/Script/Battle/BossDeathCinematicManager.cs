...
{
    [Header("監視対象の中ボス"), SerializeField]
    AITester m_BossEnemy;

    [Header("演出用カメラ"), SerializeField]
    CinemachineVirtualCamera m_TargetCamera;

    [Header("演出用アニメーター"), SerializeField]
    Animator m_CinematicAnimator;

    [Header("Barkトリガー名"), SerializeField]
    string m_BarkTriggerName = "Bark";

    [Header("演出後に有効化するオブジェクト"), SerializeField]
    GameObject m_EnableTargetObject;

    [Header("演出後に削除するオブジェクト"), SerializeField]
    GameObject m_DestroyTargetObject;
    ...
    async UniTaskVoid PlayCinematicSequence()
    {
        ...
        //アニメーション終了時バーチャルカメラを優先度0にした後、falseにする
        if (m_TargetCamera != null)
        {
            m_TargetCamera.Priority = 0;
            m_TargetCamera.gameObject.SetActive(false);
        ...
        //その後に指定したオブジェクトのSetActiveをtrueにしたい

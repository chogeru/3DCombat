...
public class PlayerController : MonoBehaviour
{
    ...
    [Header("ブリンク"), Header("ブリンク時の速度"), SerializeField]
    ...
    [Header("ダメージ設定"), Header("ダメージ後の無敵時間"), SerializeField]
    ...
    //死亡アニメーション待ち時間
    float m_DeathDelay = 3.0f;

    [Header("ダッシュ制限設定"), SerializeField]
    //最大連続回数
    int m_MaxConsecutiveDashes = 2;
    
    [SerializeField]
    //リセットまでのクールタイム
    float m_DashResetTime = 1.0f;
    
    //現在の使用回数
    int m_CurrentDashCount = 0;
    //最後にダッシュした時間
    float m_LastDashTime = -10f;

    [Header("待機設定"), SerializeField, Tooltip("待機アニメーションを開始するまでの時間(秒)")]
...
public bool IsStunned => m_PHC != null && m_PHC.IsStunned;
    ...
    private void FixedUpdate()
    {
        ...
        {
            //重力
            gravityMove.y = -9.81f * Time.fixedDeltaTime;
        }

...
{
    [Header("必要なコンポーネント"), SerializeField]
    Animator m_Animator;
    [SerializeField]
    ...
    [Header("攻撃設定"), SerializeField, Tooltip("敵の背後どれくらいの距離に出現するか")]
    //敵の後ろ1m
    float m_TeleportOffset = 1.0f;

    [Header("アニメーション設定"), SerializeField, Tooltip("攻撃のアニメーションステート名")]
    ...
    private Vector3 GetSafeTeleportPosition(Transform target)
    {
        ...
        {
            //真奥 (Pink Heart)
            targetPos + (forwardVec * m_TeleportOffset),
            //右奥
            targetPos + (forwardVec * m_TeleportOffset) + (rightVec * 0.5f),
            //左奥
            targetPos + (forwardVec * m_TeleportOffset) - (rightVec * 0.5f),
            //右横
            targetPos + (rightVec * m_TeleportOffset),
            //左横
            targetPos - (rightVec * m_TeleportOffset),
        };
        ...
        foreach (var pos in candidates)
        {
            ...
            //中心を少し上げる
            Vector3 center = checkPos + Vector3.up * 1.0f;

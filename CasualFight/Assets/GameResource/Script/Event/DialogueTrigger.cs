...
{
    [Header("Dialogue Content"), SerializeField]
    private string m_SpeakerName = "自分";
    
    [Header("複数のメッセージ（ページ）を登録できるように配列に変更"), SerializeField, TextArea]
    private string[] m_Messages = new string[] { "ここは<color=cyan>スタミナ</color>を使いそうだぞ。" };
    
    [SerializeField]
    private float m_DisplayDuration = 3.0f;

    [Header("Settings"), SerializeField, Tooltip("一度きりしか喋らないか")]

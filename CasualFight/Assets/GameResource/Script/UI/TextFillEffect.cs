...
{
    [Header("設定"), Tooltip("全体の表示にかかる時間（秒）"), SerializeField]
    float m_Duration = 1.5f;

    [Tooltip("各文字のフェードイン時間（秒）"), SerializeField]
    float m_CharFadeDuration = 0.3f;

    [Tooltip("右から左に表示するか（falseなら左から右）"), SerializeField]
    bool m_RightToLeft = true;

    [Tooltip("開始時に自動で再生するか"), SerializeField]
    bool m_PlayOnEnable = true;
